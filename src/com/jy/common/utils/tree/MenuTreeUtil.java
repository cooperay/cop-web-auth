...
public class MenuTreeUtil {
    ...
    public static String buildTreeHtml(List<Resources> menusDB){  
        ...
        for (int i=0;i<menusDB.size();i++) { 
            ...
            if ("0".equals(node.getParentId())) { 
            	...
            	html.append("\r\n<a ");
            	html.append(" href='#maincontent' style='cursor:pointer;'  onclick=\"openMenu('"+node.getType()+"','menu"+node.getId()+"','menu0','"+node.getName()+"','noset')\" ");       		    	
            	...
            	html.append("</a>");
                build(menusDB,node,html,node.getResUrl());  
                html.append("</li>");
            }  
        }  
    ...
    private static void build(List<Resources> menusDB,Resources node,StringBuffer html,String systemUrl){  
        List<Resources> children = getChildren(menusDB,node); 
        if (!children.isEmpty()) {  
            html.append("\r\n<ul class='submenu'>");
            for (Resources child : children) {  
            	...
            	html.append("\r\n<a ");
        		html.append(" href='#maincontent' style='cursor:pointer;' onclick=\"openMenu('"+child.getType()+"','menu"+child.getId()+"','menu"+child.getParentId()+"','"+child.getName()+"','"+(StringUtils.isNotBlank(child.getResUrl())?systemUrl+child.getResUrl().trim():"noset")+"')\" ");
            	if(childrenHas)html.append(" target='mainFrame' class='dropdown-toggle' "); 
            	html.append(" >"); 
            	...
            	if(childrenHas)html.append("<b class='arrow icon-angle-down'></b>");
            	html.append("</a>");
                build(menusDB,child,html,systemUrl);  
                html.append("</li>");  
            } 
            html.append("\r\n</ul>");
        }   
